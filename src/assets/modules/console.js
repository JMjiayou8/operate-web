  ...
  function sortPart () {
    layui.use(['index'], function () {
      ...
      for (var i = 0; i < config.length; i++) {
        var item = config[i];
        htmlArr.push($('#part' + item.id).html())
      }
      for (var i = 0; i < config.length; i++) {
        var item = config[i];
        var className = "";
        ...
        showArr[item.sort - 1] = '<div class="' + className + '">' + $('#part' + item.id).html() + '</div>'
      }
      var htmlStr = '<div class="layui-col-md8 layui-col-sm12" id="part0">' +
